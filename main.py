...
async def set_manager_or_admin(update):
    ...
    try:
        ...
        await bot.send_msg(id, "Неверный ввод.")
        return
    usr = get_user(id)
    if not usr:
        await bot.send_msg(id, "Такой пользователь не найден.")
...
async def set_manager(update):
    ...
    try:
        ...
        await bot.send_msg(id, f"Неверный ввод. ")
        return
    homestay = get_homestay(id_homestay)
    if not homestay:
        await bot.send_msg(id, f"Не удалось найти это место.")
        bot.clear_next_step(id)
        return
...
def show_all(name=None):
    ...
    all = [ # ПРИМЕР НА ВРЕМЯ ОТСУТСТВИЯ РЕАЛЬНОЙ ИНФЫ. УБРАТЬ!
        ...
        Homestay(1, "Улица Ленина 5", -1, -1, "9:00", "20:00",
